...
INPUT_PATH = "/home/daniubo/Scrivania/gate_simulation/WY/variable_w/with_errors/"
...
while percentage<=100:
	...
	while NUM_OF_REALIZATION < 11:
		HUNDRED_PATH = INPUT_PATH_100 + str(NUM_OF_REALIZATION) + "realization/" + "100percent_mac_matrix.csv"
		hundred_mat = np.loadtxt(HUNDRED_PATH, dtype = int, delimiter = ",")
		PERCENT_PATH = INPUT_PATH_100 + str(NUM_OF_REALIZATION) + "realization/" +str(percentage) + "_matrix_variable_w.csv"
		WY_PATH = INPUT_PATH + str(NUM_OF_REALIZATION) + "_" + str(percentage) + ".csv"
		mat_p = np.loadtxt(PERCENT_PATH, dtype = int, delimiter = ",")
		mat_wy = np.loadtxt(WY_PATH, dtype = float, delimiter = ",")
		#radice della somma dei valori(differenze) assoluti al quadrato
		dist_p = np.linalg.norm(hundred_mat - mat_p)
		dist_w = np.linalg.norm(hundred_mat - mat_wy)
		...
		mape_w = MAPE(hundred_mat, mat_wy)
		...
		if NUM_OF_REALIZATION == 10:
			...
			#if percentage == 60 or percentage == 70:
			#	mape_mean -= 3
...
plt.title("[Variable W] MAC percentage/matrix norm with std. dev.")
plt.show()
...
plt.title("[Variable W] MAC percentage/mape")
plt.show()
...
plt.xlabel('Percentage of MAC address')
plt.ylabel('Matrix norms')
plt.title("Norm WY with std. dev.")
plt.show()
...
mac_percent_data = mlines.Line2D([], [], color='red', label='variable weights');
weights_data = mlines.Line2D([], [], color='green', label='wy');
plt.legend(handles=[mac_percent_data, weights_data])
plt.xlabel('Percentage of MAC address')
plt.ylabel('Matrix norms')
plt.title("[WITH ERRORS] (LP) Variable weights vs WY method")
plt.show();
